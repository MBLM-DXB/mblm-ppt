...
| `visual_beat` | enum | `atmosphere` \| `explainer` \| `process` \| `chart` \| `comparison` \| `proof` \| `structure` \| `chrome_none` |
| `visual_job` | string | One sentence: what the visual must make the audience see |
...
| `chart_spec` | object | When `visual_beat: chart` — see VISUAL_BEATS.md (facts only) |
| `diagram_spec` | object | When `visual_beat: process` / comparison-as-diagram |
| `explainer_spec` | object | When `visual_beat: explainer` |
...
- [ ] No invented KPIs
- [ ] `visual_mode` matches chooser; filenames use `photo_` / `illustration_` / `diagram_` / `chart_` prefixes
- [ ] Every slide has `visual_beat` + `visual_job`; teaching beats ≥40% of body slides
- [ ] Charts have `chart_spec` with sourced numbers only (or METRIC NEEDED)
- [ ] Process slides declare `diagram_spec` (`layout_native` vs PNG)
