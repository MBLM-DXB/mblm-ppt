...
description: >
  ...
  to mblm-ppt-master. Owns Kimi-style visual beats (chart / process / explainer /
  atmosphere / structure) so ppt-master only executes brand freeze. Prefer
  photographic when applicable; illustration for explainers; diagram for process;
  type-only for agenda / tables / matrices. Maps content to mblm-r2 (39) + core
  kit. Use when planning an MBLM deck or preparing handoff before PPTX.
---
...
| Token | Lock |
|---|---|
| Content titles | **`BentonSansCond-Light` 40pt** in font-family (SVG **53.3** = 40/0.75); weight alone ≠ Light |
| Body | **`BentonSansCond-Regular` 16pt or 18pt** min (SVG **21.3** / **24** when exporter-scaled) |
| Weights allowed | **Light / Regular / Bold / Italic / Black** only |
| **Banned** | **Medium / MediumSC** (and Medium Italic / Medium Italic SC) |
| Section agenda section labels | **50pt Light** (SVG **66.7**) on `39_r2_40_section_agenda`; **one word each** (e.g. You, Us, Scope, Edge) — no phrases |
...
## Visual beats (Kimi-style — planner-owned)

Load [`references/VISUAL_BEATS.md`](references/VISUAL_BEATS.md) **after** the
visual chooser. For every slide, set:

| Field | Purpose |
|---|---|
| `visual_beat` | `atmosphere` \| `explainer` \| `process` \| `chart` \| `comparison` \| `proof` \| `structure` \| `chrome_none` |
| `visual_job` | One sentence: what the audience must *see* |
| `chart_spec` / `diagram_spec` / `explainer_spec` | When that beat applies (facts only for charts) |

**Hard rule:** `mblm-ppt-master` executes these fields; it must **not** re-pick
the visual role. Prefer teaching beats (`process` / `chart` / `explainer`) on
≥40% of body slides. Never invent chart data.
...
- `visual_beat` — see VISUAL_BEATS.md
- `visual_job` — one sentence teaching intent
- `image_prompt` and/or `image_filename` — when visual_mode needs an asset
- `chart_spec` / `diagram_spec` / `explainer_spec` — when the beat requires them
...
3. For each beat: pick `layout_id` via LAYOUT_PICKER; set `visual_mode` via chooser;
   then assign `visual_beat` + `visual_job` (+ chart/diagram/explainer specs) via VISUAL_BEATS.
4. Fill titles/body from source language; leave gaps marked in `notes`.
5. Rhythm-check: ≥40% of body slides teach via process/chart/explainer; no inventing KPIs.
6. Emit ordered slide map (OUTPUT_SCHEMA). Stop. Do not generate PPTX.
