...
## 2026-09-15 — `6.3.2-mblm.4` (BentonSansCond-Light PPT export)

### Fixed

- **Alumni root cause**: PPT exports PostScript face from `font-family`, not CSS weight. Bare `BentonSansCond` + `font-weight="300"` → Regular on Mac.
- Engine: `resolve_benton_cond_typeface()` maps bare `BentonSansCond` by weight → `-Light` / `-Regular` / `-Bold` / `-Black` before Writing DrawingML `a:latin typeface`. Suffixed faces and non-Benton fonts unchanged. Weight **500** → Regular (Medium banned).
- Brand preset + freeze docs: titles must name `BentonSansCond-Light` in font-family; content title **40pt** (not 47).
- SVG authoring sweep: short `BentonSansCond` in r2 templates replaced with weight-aware PostScript faces.
- Smoke: `scripts/smoke_benton_cond_typeface.py`.

### Changed

- Docs: `TYPOGRAPHY.md`, `brand_spec.md`, brand `design_spec.md`, `MBLM_OVERLAY.md`, `SKILL.md`, `LAYOUT_METRICS.md`, `RESOLVED_DECISIONS.md` §20, `skills/mblm-deck-planner/SKILL.md`.
