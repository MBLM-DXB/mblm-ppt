...
MBLM-locked recipe on top of this MBLM PPT Master package (ppt-master engine at package root). Default path is **Quick Generate** with MBLM brand constraints pre-applied (no free-design color exploration). Use **Default Generate** only when the user explicitly wants Strategist confirmation UI.

Locked brand decisions: `${SKILL_DIR}/references/mblm/RESOLVED_DECISIONS.md` (includes **v4.1.1** persistent layout lock + R2 catalog). See also `LAYOUT_METRICS.md`.

## Prerequisites

1. Read `${SKILL_DIR}/SKILL.md` and `${SKILL_DIR}/references/mblm/PATHS.md`.
2. Set:
   - `SKILL_DIR` = this skill's absolute path
   - `PPT_MASTER_SKILL` = `${SKILL_DIR}` (this package)
3. Run engine integrity gate once per session (fail closed):
   ```bash
   python3 "${PPT_MASTER_SKILL}/scripts/attribution_guard.py"
   ```
   Non-zero or missing scripts → **stop**; do **not** bypass; do **not** fall back to python-pptx.

---

## Path A — Quick Generate (default)

Uses **skill brand assets** + package-root scripts (not the official ~70MB PPTX).

Follow engine Quick profile at
`${SKILL_DIR}/workflows/profiles/quick-generate.md`
with these MBLM bindings:

### A0. Visual reference pass (MANDATORY — before brand freeze / project init / SVG authoring)

1. Read `${SKILL_DIR}/references/mblm/visual-references/CATALOG.md`.
2. Read `${SKILL_DIR}/references/mblm/visual-references/CATALOG_R2.md`.
3. Read `${SKILL_DIR}/references/mblm/visual-references/LAYOUT_PATTERNS.md`.
4. If PNG banks exist in the workspace, inspect them; otherwise rely on catalogs + `assets/mblm/layouts/`.
...
|---|---|
| Canvas | 1280×720 (`--format` only if the engine registers an exact match; else author viewBox `0 0 1280 720`); scale **37.795 px/cm** vs 33.867×19.05 cm |
| **Layout metrics** | `references/mblm/LAYOUT_METRICS.md` — cm + px locks for margins / logos / titles |
| **Margins** | **1.05 cm ≈ 39.7 px** all sides (practical **40**) — `references/mblm/MARGINS.md` |
| Primary colors | `#000000`, `#FFFFFF`, `#FFF200`, gray `#BFBFBF` |
| Neutrals / secondary | Full tables in `references/mblm/COLORS.md` |
| **Lines** | `references/mblm/LINE_RULES.md` — **no yellow lines**; lines above cards = `#002A60` / `#006AF1` / `#00AEFF` / `#0068EB` only; **white-only text on black fills** |
...
| Icons | Prefer `assets/mblm/icons/` (`ICONS.md`); always quote `MBLM_ UI-UX Design.svg` |
...
| Topic only | Run engine topic-research stage, then import the research pair |
...
**Always** install the sole indexed brand workspace (never another brand):
...
cp "${SKILL_DIR}/assets/mblm/logos/"*.svg "<project_path>/images/"
cp "${SKILL_DIR}/assets/mblm/icons/"*.svg "<project_path>/icons/"
```

Write optional `<project_path>/mblm_brand_lock.md` pointing at `references/mblm/` LAYOUT_METRICS, COLORS, LINE_RULES, TYPOGRAPHY, TEXT_BOXES, MARGINS, BULLETS, LOGOS, and chosen layout patterns (include J/K for agenda).
...
## Process diagrams (mblm-diagram)

Monoline process / methodology art uses nested skill `skills/mblm-diagram/` (allumni-diagram v2 compatible). Engine notes: `references/mblm/DIAGRAMS.md`.

**Two-turn rule (Alumni):** never stack `mblm-diagram` + `mblm-ppt-master` Skill Scripts in one turn if Alumni returns 400 spawn errors. Generate diagrams first → `projects/_host_images/diagram_<slug>.png`, then run ppt-master alone to place and export.

**Single-agent:** load `skills/mblm-diagram/SKILL.md` as Image_Generator style reference when Purpose mentions process / methodology / phases.

**§VIII / Quick image row pattern**

| Field | Value |
|---|---|
| Acquire Via | `ai` or `user` (after PNG on disk) |
| Type | Illustration |
| Crop | no-crop |
| Purpose | process diagram |
| File | `images/diagram_<slug>.png` |

Prefer R2/kit process hosts (`DIAGRAM_SLOTS.md`). Fallback SVGs: `assets/mblm/diagrams/`. Prompt helper: `scripts/mblm_diagram_prompt.py`.

## Illustration + cinematic (mblm-illustration / mblm-cinematic)

Metaphor illustrations and film-still photos use nested skills:

- `skills/mblm-illustration/` — one accent only; textured metaphors; `illustration_<slug>.png`
- `skills/mblm-cinematic/` — cool blue-teal + one warm source; `photo_<slug>.png`

Engine index: `references/mblm/IMAGES_GEN.md`. Hosts: `templates/decks/mblm-r2/IMAGE_SLOTS.md`.

**Role map:** cover/section/closing → cinematic (full-bleed + dark overlay); abstract strategy → illustration (centered, padded); process → diagram (STEP 2).

**§VIII patterns**

| Mode | Acquire Via | Type | Crop | Purpose |
|---|---|---|---|---|
| Illustration | `ai` or `user` | Illustration | no-crop (adaptive heroes) | metaphor illustration |
| Cinematic | `ai` or `user` | Photography | adaptive often | cinematic photo |

**Two-turn (Alumni):** Turn 1 generate all visuals → `projects/_host_images/`; Turn 2 ppt-master alone. Do not stack visual + ppt Skill Scripts if 400 spawn errors.

**Single-agent:** load nested SKILL.md as Image_Generator style refs. Prompt helpers:

```bash
python3 scripts/mblm_illustration_prompt.py --topic "Brand system as a city" --accent yellow --background white
python3 scripts/mblm_cinematic_prompt.py --subject "two silhouettes" --action "walking toward light" --location "urban plaza dusk" --light "low sun" --motion still --face obscured
```

Scripts emit prompts only — they do not call image APIs. Never mix illustration accents; never recolor diagrams.
