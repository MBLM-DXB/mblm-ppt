...
> **MBLM PPT Master overlay**: default brand is **always** `mblm`.
> `templates/brands/brands_index.json` lists only `mblm` — discovery cannot
> select another brand. Unless the user explicitly requests Create Template or
> Edit Native PPTX, prefer Path A via [`generate-mblm-pptx.md`](./generate-mblm-pptx.md)
> + [`profiles/quick-generate.md`](./profiles/quick-generate.md) with brand root
> `${SKILL_DIR}/templates/brands/mblm/`. Never invent McKinsey/BCG/other presets.
...
**MBLM brand default**: for Generate routes, always install `${SKILL_DIR}/templates/brands/mblm/` (the sole indexed brand). Do not offer free-design as a way to escape MBLM identity — free structure is allowed, free *identity* is not.
