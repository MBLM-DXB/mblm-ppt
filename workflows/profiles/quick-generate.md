...
> **MBLM PPT Master**: default brand workspace is
> `${SKILL_DIR}/templates/brands/mblm/`. Install it on every Quick run that
> produces a deck (via `apply_template.py` / Path A in
> [`generate-mblm-pptx.md`](../generate-mblm-pptx.md)). Brand discovery cannot
> pick another firm — `brands_index.json` contains only `mblm`. Always honor
> the MBLM brand freeze in `SKILL.md` / `MBLM_OVERLAY.md` / `references/mblm/`.
...
| Templates | Validate and install at most one exact workspace root per kind supplied for this run; **always include** `${SKILL_DIR}/templates/brands/mblm/` as the Brand root when none was supplied; before P01 inspect the complete installed SVG roster and freeze one Template Application paragraph in context; with no Layout/Deck root, free *structure* under MBLM identity (never free identity) |
