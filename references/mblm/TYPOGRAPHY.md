...
> **HARD RULE (PPT export):** PPT exports the PostScript face from `font-family`, **not** CSS weight. Titles **MUST** use `BentonSansCond-Light` in `font-family`. `font-weight="300"` alone does **NOT** produce Light — bare `BentonSansCond` + 300 resolves as Regular on Mac.
...
### Official `font-family` stacks (authoring — first token = PostScript face)

| Role | First family token (required) | Example stack |
|---|---|---|
| Titles / headlines | `BentonSansCond-Light` | `BentonSansCond-Light, Arial Narrow, Arial, sans-serif` |
| Body copy | `BentonSansCond-Regular` | `BentonSansCond-Regular, Arial Narrow, Arial, sans-serif` |
| Emphasis / topic headers | `BentonSansCond-Bold` | `BentonSansCond-Bold, Arial Narrow, Arial, sans-serif` |
| Highlighted copy / Supertext | `BentonSansCond-Black` | `BentonSansCond-Black, Arial Narrow, Arial, sans-serif` |

Do **not** author titles as `font-family="BentonSansCond"` + `font-weight="300"`. The exporter maps bare `BentonSansCond` by weight as a safety net, but templates and brand presets must name the PostScript face explicitly.
