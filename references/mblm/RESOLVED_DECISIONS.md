...
4. **Content-slide titles**: **`BentonSansCond-Light` 40pt only** (SVG **53.3**; face in font-family) — never 47/54/24–36. Agenda page title → **40pt**. Section labels on r2_40 → **50pt** Light. *(v4.2.1 overrides prior 47pt; §20 PostScript face rule.)*
...
## 20. BentonSansCond PostScript face in font-family (locked 2026-09-15 Asia/Dubai)

**Alumni root cause (verified):** `parse_project_font_weight` only sets DrawingML bold if weight ≥600; never selects Light. `parse_font_family` copies the first CSS family into `a:latin typeface`. Bare `BentonSansCond` + `font-weight="300"` therefore exports Regular on Mac.

1. **Titles MUST use** `font-family="BentonSansCond-Light, …"` (first token = PostScript face). `font-weight="300"` alone does **not** produce Light.
2. **Body** → `BentonSansCond-Regular`; **emphasis** → `BentonSansCond-Bold`; **highlight/Supertext** → `BentonSansCond-Black`.
3. **Engine safety net:** `resolve_benton_cond_typeface(family, weight)` maps bare `BentonSansCond` by weight (300→Light, 400/500→Regular, ≥600→Bold, 900→Black). Suffixed faces and non-Benton fonts unchanged.
4. Content title size remains **40pt** (SVG **53.3**); prior 47pt stays revoked.
